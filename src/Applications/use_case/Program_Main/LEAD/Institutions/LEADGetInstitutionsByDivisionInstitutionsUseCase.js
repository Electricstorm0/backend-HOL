...
class LEADGetInstitutionsByDivisionInstitutionsUseCase {
  constructor({ leadDivisionInstitutionsRepository, leadMasterDivisionRepository, leadInstitutionsRepository, leadInstitutionsClusterRepository, leadMasterInstitutionsClusterFocusRepository }) {
  ...
  async execute({ id }) {
    const { id_lead_institution: institutionsLeadId, id_lead_division: divisionLeadId } = (await this._leadDivisionInstitutionsRepository.readById({ id })) || {};
