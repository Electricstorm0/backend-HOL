...
const CLPGetUsersMe = require('../../../../../Domains/program_main/clp/users/entities/CLPGetUsersMe');

class CLPGetUsersMeUseCase {
  constructor({
    usersDetailRepository, masterProgramRoleRepository,
    offeredProgramRepository, masterBatchRepository,
  }) {
  ...
  async checkHasActiveProgramByUsersId({ id }) {
    ...
    const currentlyActiveProgram = await Promise.all(offeredProgram.map(async (value) => ({
  ...
  async execute({ id: usersId }) {
    ...
    return new CLPGetUsersMe({ ...users, currentlyProgram });
  }
}

module.exports = CLPGetUsersMeUseCase;
