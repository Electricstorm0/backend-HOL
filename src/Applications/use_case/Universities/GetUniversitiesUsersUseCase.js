...
class GetUniversitiesUsersUseCase {
  constructor({ usersUniversitiesRepository, universitiesMajorRepository, masterUniversitiesRepository, masterUniversitiesMajorRepository, masterProvinciesRepository, masterRegenciesRepository }) {
  ...
  async execute({ usersId, batchId }) {
    ...
    const { id_universities: universitiesId, id_major: majorId } = (await this._universitiesMajorRepository.readById({ id: universitiesMajorId })) || {};
    const universities = (await this._masterUniversitiesRepository.readById({ id: universitiesId })) || {};
    const { id_province: provinceId, id_regencie: regencieId } = universities;
