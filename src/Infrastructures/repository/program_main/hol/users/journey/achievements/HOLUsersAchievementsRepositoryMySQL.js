...
class HOLUsersAchievementsRepositoryMySQL extends HOLUsersAchievementsRepository {
  ...
  async create({ usersId, eventsName, eventsYear, bcfContribution, achievements }) {
    const query = {
      text: `INSERT INTO tx_hol_achievements (id_users_hol,events_name,events_year,bcf_contribution,achievements) 
      VALUES (?,?,?,?,?)`,
      values: [usersId, eventsName, eventsYear, bcfContribution, achievements],
    };
  ...
  async readByUsersId({ usersHOLId }) {
    const query = {
      text: 'SELECT * FROM `tx_hol_achievements` WHERE id_users_hol=?',
      values: [usersHOLId],
    };
    const [result] = await this._pool.query(query.text, query.values);
    return result;
