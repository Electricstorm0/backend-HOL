...
class HOLEventsBARepositoryMySQL extends HOLEventsBARepository {
  ...
  async read() {
    const query = {
      text: 'SELECT e.id_hol_events_type, e.name, e.deadline, e.duration, e.description, e.benefit, e.contact_person,e.id_regencies, eb.picture_url, eb.category FROM tx_hol_events AS e JOIN tx_hol_events_ba AS eb ON eb.id_events_hol = e.id',
    };
