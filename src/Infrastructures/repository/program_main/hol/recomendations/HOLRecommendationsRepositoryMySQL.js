const HOLRecommendationsRepository = require('../../../../../Domains/program_main/hol/temp-Recomendations/HOLRecommendationsRepository');
...
class HOLRecommendationsRepositoryMyQL extends HOLRecommendationsRepository {
  ...
  async read({ skip, numPerPage }) {
    const query = {
      text: 'SELECT concat(ud.first_name," ", ud.last_name) as Nama_Alumni,stp.name as Program, b.batch, YEAR(b.date_start) as Tahun,   r.deadline, mrs.name as status FROM `tx_hol_recomendations` as r JOIN tx_users_detail as ud on ud.id = r.id_hol_users_recommendations JOIN tx_offered_program as op on op.id_users = r.id_hol_users_recommendations JOIN master_batch as b on b.id = op.id_batch JOIN master_third_tier_program as mtp on mtp.id = op.id_third_tier_program JOIN master_second_tier_program as stp on stp.id = mtp.id_second_tier_program left JOIN tx_hol_users_recommendations_status as rs on rs.id_hol_recommendations = r.id LEFT JOIN master_hol_recommendations_status as mrs on mrs.id = rs.id_recommendations_status ORDER BY rs.id ASC LIMIT ?,?',
  ...
  async readByStatusId({ skip, numPerPage, recommendationStatusId }) {
    const query = {
      text: 'SELECT concat(ud.first_name," ", ud.last_name) as Nama_Alumni,stp.name as Program, b.batch, YEAR(b.date_start) as Tahun,   r.deadline, mrs.name as status FROM `tx_hol_recomendations` as r JOIN tx_users_detail as ud on ud.id = r.id_hol_users_recommendations JOIN tx_offered_program as op on op.id_users = r.id_hol_users_recommendations JOIN master_batch as b on b.id = op.id_batch JOIN master_third_tier_program as mtp on mtp.id = op.id_third_tier_program JOIN master_second_tier_program as stp on stp.id = mtp.id_second_tier_program right JOIN tx_hol_users_recommendations_status as rs on rs.id_hol_recommendations = r.id LEFT JOIN master_hol_recommendations_status as mrs on mrs.id = rs.id_recommendations_status WHERE rs.id_recommendations_status=? ORDER BY rs.id ASC LIMIT ?,?',
