...
class HOLUsersArticlesRepositoryMySQL extends HOLUsersArticlesRepository {
  ...
  async readCountUsersArticle() {
    const query = {
      text: 'SELECT COUNT(*) as total from `tx_hol_users_articles` ',
    };
  ...
  async readCountArticlesByUsersId({ usersHOLId }) {
    const query = {
      text: 'SELECT COUNT(*) as total from `tx_hol_users_articles` where id_users_hol = ?',
      values: [usersHOLId],
    };
  ...
  async read({ skip, numPerPage }) {
    const query = {
      text: `SELECT hua.id, CONCAT(ud.first_name," ",ud.last_name) as penulis, ma.title, hua.status 
       FROM 
       \`tx_hol_users_articles\` as hua 
       JOIN 
       \`master_articles\` as ma on ma.id = hua.id_article 
       JOIN 
       tx_users_detail as ud on ud.id = hua.id_users_hol `,
